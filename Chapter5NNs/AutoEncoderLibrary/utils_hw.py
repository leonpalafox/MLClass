...
#                           b2grad.reshape(visible_size)))
    ...
    theta = theta[hidden_size * visible_size:]
    W2 = theta[0:hidden_size * visible_size].reshape(visible_size, hidden_size)
    theta = theta[hidden_size * visible_size:]
    b1 = theta[0:hidden_size]
    b2 = theta[hidden_size:]
    theta = None

    z2 = W1.dot(data) + np.tile(b1,(data.shape[1],1)).transpose()
    a2 = sigmoid(z2) # hidden_size x num_data array.  each column is a2 for one datum
    z3 = W2.dot(a2) + np.tile(b2, (data.shape[1],1)).transpose() # visible_size x num_data
    a3 = sigmoid(z3) # visible_size x num_data
    b1 = b2 = None
    
    cost = a3 - data
    cost = np.square(cost)
    m=data.shape[1]
    indv_cost = 0.5 * np.sum(cost, axis=0)
    cost = np.sum((a3 - data) ** 2) / (2 * m) + \
           (lambda_ / 2) * (np.sum(W1 ** 2) + np.sum(W2 ** 2))
    cW2 = None
    
    delta3 = (a3 - data) * sigmoid_prime(z3)
    delta2 = (W2.T.dot(delta3)) * sigmoid_prime(z2)
    z2 = None
    
    gradW1 = delta2.dot(data.T)
    gradW1 = gradW1 / data.shape[1]
    gradW1 += lambda_ * W1
    # now do I multiply the indv_cost by gradW1 and set gradW1 to be the average of that?
    gradW2 = delta3.dot(a2.T)
    gradW2 = gradW2 / data.shape[1]
    gradW2 += lambda_ * W2
    
    gradB1 = np.sum(delta2, axis=1)/data.shape[1]
    gradB2 = np.sum(delta3, axis=1)/data.shape[1]
    
    grad = np.concatenate(( gradW1.reshape(hidden_size * visible_size),
                            gradW2.reshape(hidden_size * visible_size),
                            gradB1.reshape(hidden_size),
                            gradB2.reshape(visible_size)))
